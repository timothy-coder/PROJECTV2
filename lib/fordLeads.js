...
const FORD_CREATE_STATUSES = ["Assigned"];
const FORD_PATCH_STATUSES = ["New", "Contacted", "Closed Won", "Closed Lost", "Assigned"];
...
function normalizeFordContactForRead(contact = {}) {
  return {
    ...
    documentType: contact.documentType ?? null,
...
function validateFordLeadPayload(payload, { patch = false } = {}) {
  ...
  if (patch && !FORD_PATCH_STATUSES.includes(payload.status)) errors.push("status debe ser New, Contacted, Assigned, Closed Won o Closed Lost");
  if (!patch && !FORD_CREATE_STATUSES.includes(payload.status)) errors.push("status debe ser Assigned");
