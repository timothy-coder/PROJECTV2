...
const TRANSIENT_FORD_STATUSES = new Set([502, 503, 504]);
...
function isHtmlResponse(value) {
  ...
  return /^</.test(text) || /<html[\s>]/i.test(text) || /<body[\s>]/i.test(text);
}

function parseFordText(text) {
  if (!text) return null;
  try {
    return JSON.parse(text);
  } catch {
    return isHtmlResponse(text) ? { rawBodyType: "html" } : { message: text };
  }
}

function nestedFordMessage(payload) {
  const message = payload?.error?.message?.description
    || payload?.error?.message
    || payload?.error_description
    || payload?.message;
  if (!message) return "";
  if (typeof message === "string") return isHtmlResponse(message) ? "" : message;
  return JSON.stringify(message);
}

function userFriendlyFordError(status, payload, text) {
  const nested = nestedFordMessage(payload);
  if (nested) return nested;
  if (status === 502) return "Ford no respondio correctamente (502 Bad Gateway). Intenta nuevamente en unos minutos.";
  if (status === 503) return "Ford no esta disponible temporalmente (503). Intenta nuevamente en unos minutos.";
  if (status === 504) return "Ford demoro demasiado en responder (504). Intenta nuevamente en unos minutos.";
  if (isHtmlResponse(text)) return `Ford respondio HTTP ${status} con una pagina HTML.`;
  return text || `Ford respondio ${status}.`;
}

function sanitizeFordPayload(payload, text) {
  const clean = payload && typeof payload === "object" ? { ...payload } : {};
  if (isHtmlResponse(clean.message)) delete clean.message;
  if (isHtmlResponse(text)) clean.rawBodyType = "html";
  return clean;
}
...
export async function fordLeadsFetch(path, { request, token, search, method = "GET", body } = {}) {
  ...
  const retryCount = normalizedMethod === "GET" ? Number(envValue("FORD_API_MAX_RETRIES") || 2) : 0;
  ...
  for (let attempt = 0; attempt <= retryCount; attempt += 1) {
    ...
    const text = await response.text();
    const payload = parseFordText(text);

    if (response.ok) return payload;

    if (TRANSIENT_FORD_STATUSES.has(response.status) && attempt < retryCount) {
      await sleep(700 * (attempt + 1));
      continue;
    }

    const error = new Error(userFriendlyFordError(response.status, payload, text));
    error.status = response.status;
    error.payload = {
      ...sanitizeFordPayload(payload, text),
      upstreamStatus: response.status,
      outboundUrl: url,
      outboundBody: body || null,
    };
    throw error;
  }

  throw new Error("No se pudo conectar con Ford.");
}
