              ...
              <tr key={q.id} className={q.estado === "enviada" ? "bg-emerald-50" : "bg-white"}>
                ...
                <td className="px-3 py-3 text-right">
                  <Button size="icon" variant="outline" onClick={(event) => {
                    ...
                    setActionMenu({ quote: q, top: rect.bottom + 6, left: Math.max(rect.right - 230, 8) });
...
function QuoteActionsMenu({ state, onClose, onEdit, onAccessory, onGift, onReserve, onDuplicate, onPreview, onLink, onCancel }) {
  ...
  return (
    <>
      ...
      <div className="fixed z-[100] w-56 overflow-hidden rounded-lg border border-slate-200 bg-white text-slate-950 shadow-xl" style={{ top: state.top, left: state.left }}>
...
function BaseDialog({ title, children, onClose, onSubmit, wide }) { return <Dialog open onOpenChange={(o) => !o && onClose()}><DialogContent className={`${wide ? "max-w-4xl" : "max-w-lg"} bg-white text-slate-950`}><form onSubmit={(e) => { e.preventDefault(); onSubmit(); onClose(); }} className="space-y-3"><DialogHeader><DialogTitle>{title}</DialogTitle></DialogHeader>{children}<DialogFooter><Button type="button" variant="outline" onClick={onClose}>Cancelar</Button><Button type="submit">Guardar</Button></DialogFooter></form></DialogContent></Dialog>; }
