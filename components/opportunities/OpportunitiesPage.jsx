...
export default function OpportunitiesPage({ userPermissions, kind = "opportunity" }) {
  ...
  const clientOptions = [{ value: "", label: "Todos" }, ...data.options.clients.map((item) => ({ value: item.id, label: item.nombre }))];
    ...
    <TooltipProvider>
      <div className="min-w-0 bg-slate-50 p-3 text-slate-950 sm:p-4">
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div><h1 className="text-3xl font-bold text-violet-700">{copy.title}</h1><p className="text-sm text-slate-500">{copy.subtitle} {canViewAll ? `${copy.type} - Vista completa` : `${copy.type} - Mi vista`}</p></div>
        ...
        <section className="mb-4 rounded-lg border border-violet-200 bg-violet-50/30 p-4">
          <h2 className="mb-3 text-sm font-bold text-violet-700">Filtros</h2>
...
function TimelineView({ data, mode, setMode }) {
  ...
  return <section className="rounded-lg border border-violet-200 bg-white p-4 shadow-sm"><div className="mb-4 flex items-center justify-between"><h2 className="font-bold text-violet-700">{mode === "calendar" ? "Calendario" : "Linea de tiempo"}</h2><div className="flex gap-2"><Button size="sm" variant={mode === "timeline" ? "default" : "outline"} onClick={() => setMode("timeline")}>Linea</Button><Button size="sm" variant={mode === "calendar" ? "default" : "outline"} onClick={() => setMode("calendar")}>Calendario</Button></div></div>{mode === "timeline" ? <div className="space-y-3">{data.map((item) => <Tooltip key={item.id}><TooltipTrigger><div className="rounded-lg border border-slate-200 p-3 text-left" style={rowTimeStyle(item)}><p className="font-bold text-blue-700">{item.code} - {item.clienteNombre}</p><p className="text-xs text-slate-500">{item.nextAgenda || item.createdAt}</p><TimeStateBadge item={item} /></div></TooltipTrigger><TooltipContent><TooltipBody item={item} /></TooltipContent></Tooltip>)}</div> : <div className="grid gap-3 md:grid-cols-3">{Object.entries(byDate).map(([date, items]) => <div key={date} className="rounded-lg border border-slate-200 p-3"><h3 className="mb-2 font-bold text-violet-700">{date}</h3><div className="space-y-2">{items.map((item) => <Tooltip key={item.id}><TooltipTrigger><div className="rounded-md p-2 text-left text-xs font-bold text-slate-950" style={rowTimeStyle(item)}>{item.code}<p>{item.clienteNombre}</p></div></TooltipTrigger><TooltipContent><TooltipBody item={item} /></TooltipContent></Tooltip>)}</div></div>)}</div>}</section>;
}
